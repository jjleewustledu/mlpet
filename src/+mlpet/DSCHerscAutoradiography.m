classdef DSCHerscAutoradiography < mlpet.AutoradiographyBuilder
    ...
    methods %% GET/SET
        ...
        function m  = get.map(this)
            ...
            m('d')  = struct('fixed', 0, 'min',0.5*0.877,    'mean', this.d,  'max', fH* 1.03);
            m('f')  = struct('fixed', 1, 'min',    0.004305, 'mean', this.f,  'max',     0.01229); % 
            m('ir') = struct('fixed', 0, 'min', fL*0.1001,   'mean', this.ir, 'max', fH* 0.2780);
            m('p')  = struct('fixed', 0, 'min',0.5*0.225,    'mean', this.p,  'max', fH* 0.535); 
