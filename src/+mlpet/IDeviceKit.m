classdef (Abstract) IDeviceKit < handle & matlab.mixin.Heterogeneous
	...
	methods (Abstract, Static)
        createReferenceSources() % reference for the device
        createRadMeasurements() % manually curated data spreadsheets
        createDevice() % well-counter, automated sampler, scanner
        createDeviceCalibration()
        createDeviceData()
        createCalibratedDeviceData()        
        createCalibratedAif()
  	end 
