classdef DCVByGammas < mlbayesian.AbstractMcmcProblem 
	%% DCVBYGAMMAS   
    ...
    methods %% GET/SET
        ...
        function bt  = get.baseTitle(this)
            bt = sprintf('DCV by Gammas %s', this.pnum);
        end
        ...
        function m   = get.map(this)
            ...
            T  = this.times(end);
            m = containers.Map;
            m('a')  = struct('fixed', 0, 'min',  fL* 1.4, 'mean', this.a,  'max', fH* 2); 
            ...
            m('t0') = struct('fixed', 0, 'min',  fL*10,   'mean', this.t0, 'max', fH* T/2 );
        end
        function d   = get.dcv(this)
            assert(isa(this.dcv_, 'mlpet.DCV'));
            d = this.dcv_;
        end
    ...
    methods (Static)
        function [bsrf1,bsrf2] = solveBsrf(crvFn, hct)
            %% SOLVEBSRF
            %  Usage:  [bsrf_id1, bsrf_id2] = DCVByGammas.solveBsrf(crv_filename, Hct)
            ...
            bsrf1 = bsrf1(1:1920);
            bsrf1 = pchip(DCVByGammas.times1920, bsrf1, 1:crv.length);
            
            betadcv.catheterId = 2;
            bsrf2 = betadcv.silentBETADCV;
            bsrf2 = bsrf2(1:1920);
            bsrf2 = pchip(DCVByGammas.times1920, bsrf2, 1:crv.length);
        end
        function this = loadDCV(dcvFn)
            assert(lexist(dcvFn, 'file'));
            import mlpet.*;
            dcv = DCV.load(dcvFn);
            dcv.times = 1:dcv.length;
        ...
        function c    = gammaVariate(a, d, p, t0, t)
            ...
            idx_t0 = mlpet.DCVByGammas.indexOf(t, t0);
            c      = zeros(1, length(t));
        ...
        function this = simulateMcmc(a, c1, c2, c3, c4, d, p, q0, t0, t, map, kernel)
            import mlpet.*;       
            pseudoDcv.times = t;
            pseudoDcv.wellCounts = ...
                conv(kernel, DCVByGammas.concentrationDCV(a, c1, c2, c3, c4, d, p, q0, t0, t)); % simulated
            pseudoDcv.wellCounts = pseudoDcv.wellCounts(1:length(t));
            this = DCVByGammas(pseudoDcv);
            this = this.estimateParameters(map) %#ok<NOPRT>
        end
    end
    
	methods
 		function this = DCVByGammas(dcv)
 			%% DCVBYGAMMAS 
 			%  Usage:  this = DCVByGammas(DCV_obj)
            
 			this = this@mlbayesian.AbstractMcmcProblem(dcv.times, dcv.wellCounts); 			 
        ...
        function ed   = estimateDataFast(this, a, c1, c2, c3, c4, d, p, q0, t0)
            ed = this.concentrationDCV(a, c1, c2, c3, c4, d, p, q0, t0, this.times);
        end
        ...
        function        plotProduct(this)
            figure;
            plot(this.times,   this.itsConcentrationDCV, ...
                 this.times,   this.dcv.wellCounts, 'o');
            legend('estimated DCV',  'observed DCV');
            title(this.detailedTitle);
            xlabel(this.xLabel);
            ylabel(this.yLabel);
        end  
        function        plotParVars(this, par, vars)
            assert(lstrfind(par, properties('mlpet.DCVByGammas')));
    ...
    properties (Access = 'private')
        ...
        kernelBestFilename_ = '/Users/jjlee/MATLAB-Drive/mlpet/data/bsrf120_id1.mat'
                             ...
                             %'/Users/jjlee/MATLAB-Drive/mlarbelaez/src/+mlarbelaez/kernel57.mat'
    end
    ...
    methods (Access = 'private')  
        function plotParArgs(this, par, args, vars)
            ...
            for v = 1:size(args,2)
                argsv = args{v};
                ordinate = DCVByGammas.concentrationDCV(argsv{:});
                if (strcmp('q0', par))
